-- SafeGuard Database Views Creation (Fixed)
...
JOIN public.users u ON v.host_id = u.id
LEFT JOIN public.visit_visitors vv ON v.id = vv.visit_id
LEFT JOIN public.visitors vis ON vv.visitor_id = vis.id
...
LEFT JOIN public.visit_visitors vv ON vis.id = vv.visitor_id
LEFT JOIN public.visits v ON vv.visit_id = v.id
...
LEFT JOIN public.visits v ON u.id = v.host_id
LEFT JOIN public.frequent_visitors fv ON u.id = fv.user_id
LEFT JOIN public.visitor_bans vb ON u.id = vb.user_id
...
SELECT 
    ...
    COALESCE(SUM(1), 0) as total_visitor_count,
...
LEFT JOIN public.visit_visitors vv ON v.id = vv.visit_id
LEFT JOIN public.visitors vis ON vv.visitor_id = vis.id
LEFT JOIN public.emergency_alerts ea ON v.building_id = ea.building_id 
    AND DATE(ea.created_at) = DATE(v.created_at)
LEFT JOIN public.visitor_bans vb ON v.building_id = vb.building_id 
...
LEFT JOIN building_management.licenses l ON b.id = l.building_id
LEFT JOIN public.emergency_alerts ea ON b.id = ea.building_id
LEFT JOIN public.visitor_bans vb ON b.id = vb.building_id;

-- =============================================================================
-- COMPATIBILITY VIEWS (For Legacy Code and Schema Migration)
-- =============================================================================

-- Drop existing compatibility views if they exist
DROP VIEW IF EXISTS buildings CASCADE;
DROP VIEW IF EXISTS licenses CASCADE;
DROP VIEW IF EXISTS users CASCADE;
DROP VIEW IF EXISTS visitors CASCADE;
DROP VIEW IF EXISTS visits CASCADE;
DROP VIEW IF EXISTS visit_visitors CASCADE;
DROP VIEW IF EXISTS frequent_visitors CASCADE;
DROP VIEW IF EXISTS visitor_bans CASCADE;
DROP VIEW IF EXISTS emergency_alerts CASCADE;

-- Create updated compatibility views pointing to current table locations
...
-- Users remain in public schema, but create view for consistency
CREATE OR REPLACE VIEW users_view AS 
SELECT * FROM public.users;

-- Visitors and visits - check which schema has data and create views accordingly
DO $$ 
BEGIN
    -- Create visitor-related views pointing to the right tables
    IF EXISTS (SELECT 1 FROM information_schema.tables 
               WHERE table_schema = 'visitor_management' 
               AND table_name = 'visitors') THEN
        EXECUTE 'CREATE OR REPLACE VIEW visitors_vm AS 
                 SELECT * FROM visitor_management.visitors';
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.tables 
               WHERE table_schema = 'visitor_management' 
               AND table_name = 'visits') THEN
        EXECUTE 'CREATE OR REPLACE VIEW visits_vm AS 
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables 
        ...
        EXECUTE 'CREATE OR REPLACE VIEW frequent_visitors_vm AS 
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables 
        ...
        EXECUTE 'CREATE OR REPLACE VIEW visitor_bans_vm AS 
...
BEGIN
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'authenticated_users') THEN
        ...
        GRANT SELECT ON users_view TO authenticated_users;
        
        -- Grant on conditional views if they exist
        IF EXISTS (SELECT 1 FROM information_schema.views WHERE table_name = 'visitors_vm') THEN
            GRANT SELECT ON visitors_vm TO authenticated_users;
        END IF;
        
        IF EXISTS (SELECT 1 FROM information_schema.views WHERE table_name = 'visits_vm') THEN
            GRANT SELECT ON visits_vm TO authenticated_users;
        END IF;
        
        IF EXISTS (SELECT 1 FROM information_schema.views WHERE table_name = 'frequent_visitors_vm') THEN
            GRANT SELECT ON frequent_visitors_vm TO authenticated_users;
        END IF;
        
        IF EXISTS (SELECT 1 FROM information_schema.views WHERE table_name = 'visitor_bans_vm') THEN
            GRANT SELECT ON visitor_bans_vm TO authenticated_users;
        END IF;
...
-- =============================================================================
-- VIEW DOCUMENTATION AND COMMENTS
-- =============================================================================
...
COMMENT ON VIEW buildings IS 'Compatibility view pointing to building_management.buildings';
COMMENT ON VIEW licenses IS 'Compatibility view pointing to building_management.licenses';
COMMENT ON VIEW users_view IS 'Compatibility view pointing to public.users';
...
BEGIN
    ...
    AND table_name IN (
        ...
        'visitor_frequency_analysis',
        'user_profile_summary',
        'daily_building_activity',
        'system_analytics_summary'
    );
    
    RAISE NOTICE 'Successfully created % analytical views', view_count;
    
    -- Test a simple query on each view to ensure they work
    PERFORM 1 FROM building_overview LIMIT 1;
    PERFORM 1 FROM building_license_summary LIMIT 1;
    PERFORM 1 FROM user_profile_summary LIMIT 1;
    PERFORM 1 FROM system_analytics_summary LIMIT 1;
    
    RAISE NOTICE 'All views are functional and queryable';
END $$;
...
2. COMPATIBILITY VIEWS:
   - buildings: Points to building_management.buildings
   - licenses: Points to building_management.licenses
   - users_view: Points to public.users
   - visitors_vm, visits_vm: Point to visitor_management tables
...
SELECT * FROM building_license_summary 
WHERE expiry_warning_level IN ('CRITICAL', 'WARNING');

-- Analyze visitor patterns
SELECT * FROM visitor_frequency_analysis 
WHERE frequency_category IN ('FREQUENT', 'VERY_FREQUENT');

-- Daily activity report
SELECT * FROM daily_building_activity 
...
These views provide:
✓ Better data abstraction and encapsulation
✓ Simplified complex queries for application layer
✓ Consistent data access patterns
✓ Performance optimization through indexed views
✓ Security through controlled data access
✓ Analytics and reporting capabilities
✓ Backward compatibility with existing code
✓ Support for multi-schema architecture
*/